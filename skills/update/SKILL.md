...
- **`0` — ambiguous.** No merges unique to the workspace, which looks identical for a rebase-strategy workspace (a linear customization stack) *and* a fresh install that has never taken an update. The log can't tell them apart, but this workspace has a recorded convention: **default to rebase** — integrate by rebase without asking.

So the log can only ever *confirm* the merge strategy; a `0` count is "unknown," never "rebase" on its own (which is why this workspace's default was recorded here in Step 4, per Step 6).
