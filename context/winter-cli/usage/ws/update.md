...
`.winter/config.lock`. For the rest of the family, see the [`winter ws` hub](./index.md).
...
scoped to an env), so a name containing `/` is rejected. A literal name that doesn't match a pinned standalone raises a
clear error (see [Errors](#errors)); a glob matching zero pinned standalones is a no-op. Repos without a `ref` are
ignored regardless of scope.
