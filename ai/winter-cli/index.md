...
- **`winter service <action> [<env>/<service>…]`** — a stable `up`/`down`/`status`/`restart`/`logs` interface that dispatches to the orchestrator extension(s) the workspace registers. A single provider is dispatched directly; multiple bound providers are fanned out on `up` (abort on first failure) and `down` (best-effort, continues past failures), with no ordering or readiness semantics *between providers*. Opt into a winter-side readiness gate with `up --wait`, which polls the merged `status` until no service is unhealthy (see [usage/service.md](./usage/service.md)). `up`/`down` take a single `<env>`; `status`/`restart`/`logs` take `<env>/<service>` PATTERNS. All five also accept the reserved `workspace` scope (see [usage/service.md](./usage/service.md)).
