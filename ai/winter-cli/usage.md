...
|------|--------------------|
| `winter ws init` | `projects/` and standalone repos — clones missing repos, applies git identity, writes git-excludes, runs each repo's `cmd` list, and processes extensions per `adopt_extensions` |
| `winter ws init <name>` | the `./<name>/` feature worktree — creates the directory and per-repo `git worktree` on branch `<name>`, copies git identity, writes git-excludes, symlinks workflow scripts, runs each repo's `cmd` list |
| `winter ws init --all` | `projects/`, standalone repos, plus every existing worktree (detected via `git worktree list`) |
