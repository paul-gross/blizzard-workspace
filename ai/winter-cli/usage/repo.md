...
**`--ref`** (standalone only) — pins the standalone repo to a branch, tag, or commit SHA. Accepted only when `--standalone` is also given; rejected for project repos. Writes `ref = "<value>"` into the `[[standalone_repository]]` entry. See [setup.md — `ref`](./setup.md#ref--standalone-repo-pins) for the full pin semantics.
