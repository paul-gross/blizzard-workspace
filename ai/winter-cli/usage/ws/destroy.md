...
4. **Env cleanup** — removes the env directory, strips the matching `# >>> winter-dir/<env>` block from the workspace's `.git/info/exclude`, and removes the env's index entry from `.winter/state.toml`.
