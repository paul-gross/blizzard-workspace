...
Coverage:
- Sets all four base winter extension context variables.
...
def test_build_extension_env_sets_all_four_vars() -> None:
    """build_extension_env returns a dict with all four base winter extension vars."""
...
def test_build_extension_env_uses_os_environ_when_no_base() -> None:
    ...
    # os.environ keys that don't overlap with the four vars should appear.
    for key, value in os.environ.items():
        if key not in {"WINTER_WORKSPACE_DIR", "WINTER_EXT_DIR", "WINTER_EXT_PREFIX", "WINTER_EXT_CONFIG_DIR"}:
...
def test_fan_out_up_emits_winter_ext_config_dir() -> None:
    ...
    svc = ServiceFanOutService(subprocess_runner=runner, workspace_root=WS)
