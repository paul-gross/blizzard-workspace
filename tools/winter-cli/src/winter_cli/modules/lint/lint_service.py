...
class LintService:
    """Aggregates lint findings from the workspace script and every extension's script.

    A pure dispatcher: it owns discovery, ordering, and aggregation, but never
    inspects content itself — every finding originates in a contributed script.
    The workspace check runs first (mirroring the doctor `[project]`-before-
    extensions ordering), then each extension in standalone-repo order.
    """
